...
(def triv-pas
  {:text "{ trivial test program        -- file triv.pas }
   ...
   :ast '(program "graph1"
                  (progn "output")
                  (comment "defined variables" "toplevel.graph1/i")
                  (progn (:= "toplevel.graph1/i" 3)))
   :symbols [{:name "i" :type/data "integer" :qname "toplevel.graph1/i"}]})
  ...
  {:text "program graph1(output);
   ...
   :ast '(program "graph1"
                  ...
                  (comment "defined variables"
                           "toplevel.graph1/i"
                           "toplevel.graph1/lim")
                  (progn (:= "toplevel.graph1/lim" 7)
                         (progn (label 1)
                                (:= "toplevel.graph1/i" 0)
                                (if (<= "toplevel.graph1/lim" "toplevel.graph1/i")
                                  (goto 2))
                                (funcall "writeln" "toplevel.graph1/str_1")
                                (:= "toplevel.graph1/i" (+ 1 "toplevel.graph1/i"))
                                (goto 1)
                                (label 2))))
   :symbols [{:name "i" :type/data "integer" :qname "toplevel.graph1/i"}
             {:name "lim" :type/data "integer" :qname "toplevel.graph1/lim"}]})
