...
(defn- -use-reg
  ([state reg] (-use-reg state reg free-regs used-regs))
  ([state reg free-key used-key]
     (-> state
         (update-in [free-key] difference (set [reg]))
         (update-in [used-key] union (set [reg])))))
...
(defn use-reg
  ...
  ([state register] (use-reg state register free-regs used-regs))
  ([state register free-key used-key]
     (let [free (free-key state)
           used (used-key state)]
       (cond (not (register? state register))
             state

             (contains? free register)
             (-use-reg state register free-key used-key)
...
(defn free-reg
  "Deallocates a register, returning a new state record in which the argument
   ...
   marked used."
  ([state register] (free-reg state register :integer))
  ([state register type]
     (case type
       (:integer)
       ...
       (:float :real)
           (if (register? state register)
             (-> state
                 (update-in [free-float-regs] union (set [register]))
                 (update-in [used-float-regs] difference (set [register])))
             state))))
...
(defn reg-alloc
  ...
  ([state type]
     (case type
       ...
       (:float :real)
           (let [free (free-float-regs state)]
             (if (empty? free)
               [state nil]
               [(use-reg state (first free) free-float-regs used-float-regs)
                (first free)])))))
...
(defn loadlit
  ...
  (case (typeof lit)
    ...
    ("integer" nil)
        (let [[state dst] (reg-alloc state :integer)]
          [(-> state
               (free-reg dst))
